...
CREATE TABLE stud_cours_appl (
  ...
  stud_id            INTEGER  NULL     COMMENT '학생번호', -- 학생번호
  idt                DATETIME NOT NULL COMMENT '신청일', -- 신청일
  stat               BOOLEAN  NULL     COMMENT '신청결과', -- 신청결과
  pay_stat           BOOLEAN  NULL     COMMENT '결제상태', -- 결제상태
  COL2               INTEGER  NULL     COMMENT '신청결과번호' -- 신청결과번호
)
COMMENT '수강신청';
...
-- 주소
CREATE TABLE addr (
  addr_no  INTEGER      NOT NULL COMMENT '주소번호', -- 주소번호
...
CREATE TABLE cours (
  ...
  user_id  INTEGER      NULL     COMMENT '매니저번호' -- 매니저번호
)
COMMENT '교육과정';
...
-- 매니저
CREATE TABLE mgr (
  user_id INTEGER     NOT NULL COMMENT '매니저번호', -- 매니저번호
  COL37   VARCHAR(50) NOT NULL COMMENT '직위', -- 직위
  COL     VARCHAR(30) NULL     COMMENT '팩스' -- 팩스
)
COMMENT '매니저';
...
CREATE TABLE cours_subj (
  subj_id INTEGER NOT NULL COMMENT '교육과정번호', -- 교육과정번호
...
-- 결제유형
CREATE TABLE TABLE (
  pay_type           INTEGER     NULL COMMENT '결제유형번호', -- 결제유형번호
  COL                VARCHAR(50) NULL COMMENT '결제유형명', -- 결제유형명
  stud_cours_appl_id INTEGER     NULL COMMENT '수강신청번호' -- 수강신청번호
)
COMMENT '결제유형';

-- 신청결과
CREATE TABLE TABLE2 (
  COL2 INTEGER     NOT NULL COMMENT '신청결과번호', -- 신청결과번호
  COL  VARCHAR(50) NULL     COMMENT '상태' -- 상태
)
COMMENT '신청결과';

-- 신청결과
ALTER TABLE TABLE2
  ADD CONSTRAINT PK_TABLE2 -- 신청결과 기본키
    PRIMARY KEY (
      COL2 -- 신청결과번호
    );
...
-- 수강신청
ALTER TABLE stud_cours_appl
  ADD CONSTRAINT FK_TABLE2_TO_stud_cours_appl -- 신청결과 -> 수강신청
    FOREIGN KEY (
      COL2 -- 신청결과번호
    )
    REFERENCES TABLE2 ( -- 신청결과
      COL2 -- 신청결과번호
    );
...
-- 결제유형
ALTER TABLE TABLE
  ADD CONSTRAINT FK_stud_cours_appl_TO_TABLE -- 수강신청 -> 결제유형
    FOREIGN KEY (
      stud_cours_appl_id -- 수강신청번호
    )
    REFERENCES stud_cours_appl ( -- 수강신청
      stud_cours_appl_id -- 수강신청번호
    );